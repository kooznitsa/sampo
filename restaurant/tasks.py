...
from restaurant.services import DriverManager, MenuScraper, RestaurantScraper
...
@shared_task(bind=True, autoretry_for=(Exception,), retry_backoff=True, max_retries=3)
def scrape_restaurant_task(self: Task, restaurant_id: int) -> dict | None:
    restaurant = get_object_or_404(Restaurant, pk=restaurant_id)
    ...
    try:
        ...
        RestaurantScraper(driver, driver_manager.timeout, restaurant.menu_url).run()
        return {'status': 'success'}
    except Exception as e:
        error_logger.error(f'Scraping restaurant ID={restaurant_id} failed: {e}')
    finally:
        driver_manager.quit()
    return None
